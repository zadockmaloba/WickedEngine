...
namespace wi
{
	...
	void HairParticleSystem::CreateRenderData()
	{
		...
		if (particleCount > 0)
		{
			...
			bd.size =
				AlignTo(AlignTo(indirect_view.size, alignment), (uint64_t)sizeof(IndirectDrawArgsIndexedInstanced)) + // additional structured buffer alignment
				AlignTo(AlignTo(simulation_view.size, alignment), (uint64_t)sizeof(PatchSimulationData)) + // additional structured buffer alignment
			...
			buffer_offset = AlignTo(buffer_offset, (uint64_t)sizeof(IndirectDrawArgsIndexedInstanced)); // additional structured buffer alignment
			...
			buffer_offset = AlignTo(buffer_offset, (uint64_t)sizeof(PatchSimulationData)); // additional structured buffer alignment
			buffer_offset = AlignTo(buffer_offset, alignment);
