...
namespace wi::scene
{
	...
	void MeshComponent::CreateRenderData()
	{
		...
		bd.size =
			...
			AlignTo((uint64_t)uv_count * sizeof(Vertex_UVS), alignment) +
	...
	void MeshComponent::CreateStreamoutRenderData()
	{
		...
		desc.size =
			AlignTo((uint64_t)vertex_positions.size() * sizeof(Vertex_POS32), alignment) + // pos
			AlignTo((uint64_t)vertex_positions.size() * sizeof(Vertex_POS32), alignment) + // prevpos
			AlignTo((uint64_t)vertex_normals.size() * sizeof(Vertex_NOR), alignment) +
			AlignTo((uint64_t)vertex_tangents.size() * sizeof(Vertex_TAN), alignment)
			;
